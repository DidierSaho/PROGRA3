...
# Mejorar la evaluación mediante sumas parciales
def evaluar_combinacion(combinacion):
    global mejor_suma_total, mejor_combinacion
    suma_centros = sum(costos_centros[i] for i, activo in enumerate(combinacion) if activo)
    suma_total = suma_centros
    for i, nodo_v in enumerate(nodos_verticales):
